...
export const PropertyFormUpdate = () => {
  ...
  const getOneProperties = async () => {
    try {
      const url = ApiBack.PROPERTY_VIEW_ONE + "/" + propertyId;
      const data = await ServicePrivate.petitionGET(url);
      // myObject.propertyId = data.propertyId;
      // myObject.title = data.title;
      // myObject.description = data.description;
      // myObject.price = data.price;
      // myObject.address = data.address;
      // myObject.city = data.city;
      // myObject.state = data.state;
      // myObject.propertyType = data.propertyType;
      // myObject.images = [];
      // myObject.neighborhoodId = 0;
      // myObject.estratoSocial = data.estratoSocial;
      // myObject.areaConstruida = data.areaConstruida;
      // myObject.bannos = data.bannos;
      // myObject.habitaciones = data.habitaciones;
      // myObject.parqueadero = data.parqueadero;
      // console.log(data);
  ...
  const sendForm = async (fh: formaHtml) => {
    ...
    if (formulario.checkValidity() === false) {
      ...
      myObject.images = fileBase64List.map(
        (base64, index) => new PropertyImages(0, 0, base64, `image${index}`)
      );
      const urlSend = ApiBack.PROPERTY_VIEW_UPDATE + "/" + propertyId;
      const resultado = await ServicePrivate.peticionPUT(urlSend, myObject);
                      ...
                      <Form.Group className="mb-3" controlId="neighborhoodId">
                        ...
                        >
                          ...
                          {neighborhood.map((neig) => (
                            <option
                              key={neig.neighborhoodId}
                              value={neig.neighborhoodId}
                            >
                              {neig.name}
                            </option>
                          ))}
