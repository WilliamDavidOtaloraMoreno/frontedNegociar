...
export const PropertyForm = () => {
  ...
  const sendForm = async (fh: formaHtml) => {
    ...
    if (formulario.checkValidity() === false) {
      ...
      myObject.images = fileBase64List.map((base64, index) => new PropertyImages(0, 0, base64, `image${index}`));
      const resultado = await ServicePrivate.peticionPOST(
      ...
      if (resultado) {
        setEnProceso(false);
        MessageToastify("info", "Propiedad creada con éxito", 7000);
      } else {
        MessageToastify("error", "La propiedad no se creó con éxito", 7000);
      }
    }
  };
